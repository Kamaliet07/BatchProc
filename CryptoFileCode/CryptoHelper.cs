...
namespace CryptoUtility
{
    public class CryptoHelper
    {
        ...
        /// <returns></returns>
        public static string GenerateHashFile(string inFile, string outFolder)
        {
            //Generate Hash
            string hashValue = GenerateHash(inFile);
            string hashFileFolder = string.Empty;
            if (!(string.IsNullOrEmpty(outFolder)))
            {
                hashFileFolder = outFolder;
            }
            else
            {
                hashFileFolder = Path.GetDirectoryName(inFile);
            }

            string hashFileName = Path.GetFileName(inFile);
            string hashFile = hashFileFolder + "\\" + hashFileName + ".hash";

            StreamWriter sw = new StreamWriter(hashFile, false);
            sw.Write(hashValue);
            sw.Close();
        ...
        public static string GenerateHash(string inFile)
        {
            ...
            using (FileStream file = new FileStream(inFile, FileMode.Open, FileAccess.Read))
            {
                ...
                {
                    bytesHash = hashProvider.ComputeHash(file);
                }
                file.Close();
            }
            string hashValue = Convert.ToBase64String(bytesHash);

            return hashValue;
        }

        /// <summary>
        /// Determines whether [is valid password] [the specified STR password].
        /// </summary>
        /// <param name="strPassword">The STR password.</param>
        /// <returns>
        /// 	<c>true</c> if [is valid password] [the specified STR password]; otherwise, <c>false</c>.
        /// </returns>
        public static bool IsValidPassword(string strPassword)
        {
            //The following regular expression is for Validating a strong password. 
            //It must be between 4 and 10 characters, contain at least one digit and one alphabetic character, and must not contain special characters.

            Regex compareRegex = new Regex(@"(?!^[0-9]*$)(?!^[a-zA-Z]*$)^([a-zA-Z0-9]{4,10})$");
    ...
    /// </summary>
    public class CryptoHelperException : ApplicationException
    {
        public CryptoHelperException(string msg) : base(msg) { }
    }
